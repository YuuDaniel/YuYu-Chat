...
def autenticar_ad(usuario, senha):
    ...
    try:
        ...
        conn   = Connection(server, user=user_login_simple, password=senha,
                            authentication=SIMPLE, auto_bind=True)

        base_dn = 'dc=' + AD_DOMAIN.replace('.', ',dc=')

        # escape_filter_chars previne LDAP Injection — uma linha, zero impacto no uso normal
        filtro = f'(sAMAccountName={escape_filter_chars(user_short)})'
                ...
                for grupo_dn in grupos:
                    ...
                    if partes:
                        ...
                        if primeira_parte.upper().startswith('CN='):
                            lista_grupos.append(primeira_parte[3:].strip())
        ...
        registrar_log("LOGIN_SUCESSO",
                      f"User: {nome_real} | Cargo: {cargo_exibicao} | "
                      f"Equipe: {equipe_final} | Path: {caminho_completo}")
